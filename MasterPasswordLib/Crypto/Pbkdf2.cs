// Code taken from https://github.com/ChrisMcKee/cryptsharp
...
namespace MasterPassword.Crypto
{
	...
	public class Pbkdf2 : Stream
	{
		...
		private byte[] _block, _blockT1, _blockT2;
		private ComputeHmacCallback _computeHmacCallback;
		private int _iterations;
		private byte[] _key, _saltBuf;
		...
		private static void Clear(Array arr)
		{
		...
		public static ComputeHmacCallback CallbackFromHmac<T>() where T : KeyedHashAlgorithm, new()
		{
			return delegate(byte[] key, byte[] data, byte[] output)
			{
				using (var hmac = new T())
				{
					...
					hmac.Key = key;
					byte[] hmacOutput = hmac.ComputeHash(data);
		...
		private void ComputeHmac(byte[] data, byte[] output)
		{
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			while (count > 0)
			{
				...
				int bytesThisTime = Math.Min(_block.Length - bytesSoFar, count);
		...
		public override long Seek(long offset, SeekOrigin origin)
		{
			...
			switch (origin)
			{
				...
				break;
				default:
				throw new ArgumentException("Unknown seek type.", "origin");
			}

			if (pos < 0)
			{
				throw new ArgumentException("Seeking before the stream start.", "offset");
			}
			Position = pos;
			return pos;
		}
